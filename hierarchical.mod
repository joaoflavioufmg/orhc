...
param d1{I,K}:= Uniform(30,100); # The travel distance (or time) between demand point i ∈ I and a level-1 PCF in candidate location k ∈ K .
param d2{K,J}:= Uniform(30,100); # The travel distance (or time) between a level-1 PCF in candidate location k ∈ K and a level-2 PCF in candidate location j ∈ J.
param w{I}:= Normal(50,10); # The population size at demand point i ∈ I (The demand point at i ∈ I).
param p:=4; # The number of alevel-1 PCFs to be established.
param q:=2; # The number of alevel-2 PCFs to be established.
param C1{K}:= (Normal(50,10)*1.2*card(I))/p; # Uniform(100,220); # The capacity of a level-1 PCF in candidate location k ∈ K.
param C2{J}:= (Normal(50,10)*1.2*card(K))/q;# Uniform(1600,2000); # The capacity of a level-2 PCF in candidate location j ∈ J.
display C1;
display C2;
param O{K}:= 0.7; # The proportion of patients in a level-1 PCF at candidate location k ∈ K referred to a level-2 PCF.
check{k in K}: O[k] >=0;
check{k in K}: O[k] <=1; 

var x1{k in K}, >=0, binary; # 1, if a level-1 PCF is established at candidate location k ∈ K ; 0 otherwise.
var x2{j in J}, >=0, binary; # 1, if a level-2 PCF is established at candidate location j ∈ J ; 0 otherwise.
var u{i in I, k in K}, >=0;  # The flow of patients between demand point i ∈ I and a level-1 PCF at candidate location k ∈ K
var v{k in K, j in J}, >=0;  # The flow of patients referred form a level-1 PCF at candidate location k ∈ K to a level-2 PCF at candidate location j ∈ J.
...
# Constraints (29) show that the entire population of patients at each demand point must be assigned to level-1 PCFs
s.t. R29{i in I}: sum{k in K}u[i,k] = w[i];

#  Constraints (30) stipulate that θ[k] proportion of patients in a level-1 PCF are referred to open level-2 PCFs.
...
# Constraints (33) and (34) specify the total number of level-1 and level-2 PCFs to be established.
