...
##################
param f{J}:= Normal(100,10); # The fixed charge of establishing a facility at candidate location j ∈ J.
param v:= 5; # The transportation cost per item per distance unit (the variable transportation cost).
##################

param U{J}:= Uniform(80,120); # facility Capacity (In a capacitated Fixed-charge Location Problem)
...
# The objective (23) minimizes the total cost which includes the facility-opening and transportation costs.
minimize OF23: sum{j in J}f[j]*x[j] + v*sum{i in I, j in J}w[i]*d[i,j]*y[i,j];

# Constraints (24) ensure that each demand node is assigned to an open facility,
s.t. R24{i in I}: sum{j in J}y[i,j] = 1;

# Constraints (25) restrict assignments to open facilities.
s.t. R25{i in I, j in J}: y[i,j] <= x[j]; 

# In a capacitated Fixed-charge Location Problem (FCLP), a new parameter Uj is defined as the maximum capacity of each facility j
